...
## Supported Legacy Add-ons

These separate plugins have been observed to depend on the legacy/compat surface during the v15 transition:
...
- Some add-ons still call `WP_STATISTICS\\DB::table('...')` with v14 keys (e.g. `visitor`, `pages`, `visitor_relationships`).
- The legacy `search`/`visit` tables existed historically in older versions, but recent code may no longer include them in the compat table registry. If a legacy add-on still queries `DB::table('search')`, it must be migrated.
- Removing legacy classes/tables should be preceded by migrating these add-ons to v15 services (AnalyticsQuery) or providing adapters.

## Legacy Add-on Table Keys Observed

These are `WP_STATISTICS\\DB::table('...')` keys observed in legacy add-ons (excluding `wp-statistics-premium`).
This is intended as a migration checklist: each key indicates a v14 schema dependency.

| Table key | Observed in add-ons | Example location |
|---|---|---|
| `pages` | wp-statistics-rest-api, wp-statistics-widgets, wp-statistics-data-plus | `wp-statistics-data-plus/src/Services/Widgets/PostTypeManager.php:25` |
| `search` | wp-statistics-rest-api | `wp-statistics-rest-api/includes/rest-api/v1/class-searches-controller.php:207` |
| `visitor_relationships` | wp-statistics-data-plus, wp-statistics-rest-api | `wp-statistics-rest-api/includes/rest-api/v1/class-pages-controller.php:131` |
| `visitor` | wp-statistics-rest-api, wp-statistics-advanced-reporting, wp-statistics-data-plus | `wp-statistics-rest-api/includes/rest-api/v1/class-referrers-controller.php:89` |
