wps_js.summary_meta_box = {
    ...
    summary_statistics: function (args = []) {
        ...
        if (wps_js.is_active('visitors') || wps_js.is_active('visits')) {
            t += `<tr><th width="60%"></th>`; /* @todo Add "Time" to first column. Or can I add it by myself? */
            ...
            for (let i = 0; i < summary_item.length; i++) {
                ...
                ["visitors", "visits"].forEach(function (key) {
                    t += `<td class="th-center">` + (wps_js.is_active(key) ? `<a href="${args[key][summary_item[i]]['link']}"><span>${args[key][summary_item[i]]['value']}</span></a>` : ``) + `</td>`;
                });
                t += `</tr>`;
            }
            t += '</tbody>';
