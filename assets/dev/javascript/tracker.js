(function () {
    ...
    let WP_Statistics_Last_Url = window.location.href;
    ...
    let wpStatisticsTrackUrlChange = () => {
        if (window.location.href !== WP_Statistics_Last_Url) {
            WP_Statistics_Last_Url = window.location.href; // Update the last visited URL

            // Execute the sendHitRequest() on URL change
            wpStatisticsUserOnline.sendHitRequest();

            // Execute the init() on URL change
            if (WP_Statistics_Tracker_Object.option.consentLevel == 'disabled' || WP_Statistics_Tracker_Object.option.trackAnonymously ||
                !WP_Statistics_Tracker_Object.option.isWpConsentApiActive || wp_has_consent(WP_Statistics_Tracker_Object.option.consentLevel)) {
                wpStatisticsUserOnline.init();
            }

            // Handle consent change (in case consent is updated dynamically)
            document.addEventListener("wp_listen_for_consent_change", function (e) {
                const changedConsentCategory = e.detail;
                for (let key in changedConsentCategory) {
                    if (changedConsentCategory.hasOwnProperty(key)) {
    ...
    const WP_Statistics_Original_Push_State = history.pushState;
    history.pushState = function (state, title, url) {
        WP_Statistics_Original_Push_State.apply(history, arguments);
        wpStatisticsTrackUrlChange();
    };

    const WP_Statistics_Original_ReplaceState = history.replaceState;
    history.replaceState = function (state, title, url) {
        WP_Statistics_Original_ReplaceState.apply(history, arguments);
        wpStatisticsTrackUrlChange();
    };
})();