import { phpToMomentFormat } from "../helper";
...
if (wps_js.global?.page?.file === "index.php" || wps_js.is_active("overview_page") || wps_js.global?.page?.file === "post-new.php" || (wps_js.global?.page?.file === "post.php" && wps_js.isset(wps_js.global, "page", "ID"))) {
    ...
    class DatePickerHandler {
        ...
        initializeFilterToggles() {
            jQuery(document)
                .off("click", ".js-filters-toggle")
                .on("click", ".js-filters-toggle", (e) => {
        ...
        initializeMoreFilters() {
            jQuery(document)
                .off("click", ".js-show-more-filters")
                .on("click", ".js-show-more-filters", (e) => {
                    e.preventDefault();
                    jQuery(e.currentTarget).closest(".c-footer__filters__list").find(".js-more-filters").addClass("is-open");
                });

            jQuery(document)
                .off("click", ".js-close-more-filters")
                .on("click", ".js-close-more-filters", (e) => {
                    e.preventDefault();
        ...
        initializeCustomDatePicker() {
            jQuery(document)
                .off("click", 'button[data-filter="custom"]')
                .on("click", 'button[data-filter="custom"]', (e) => {
                    const $target = jQuery(e.currentTarget);
        ...
        handleDateSelection(metaboxId, dates, picker, isHide = false) {
            ...
            const dateRangeText = dates.startDate === dates.endDate ? picker.startDate.format("MMM D, YYYY") : picker.startDate.format("MMM D, YYYY") + " - " + picker.endDate.format("MMM D, YYYY");
        ...
        initializeFilterClicks() {
            jQuery(document)
                .off("click", '.c-footer__filters__list-item:not(.js-show-more-filters):not(.js-close-more-filters):not([data-filter="custom"])')
    ...
    function loadMetaBoxData(metaBoxKey, startDate = null, endDate = null, date_filter = null) {
        return new Promise((resolve, reject) => {
            ...
            wps_js.ajaxQ(wps_js.global.admin_url + "admin-ajax.php", data, successHandler, errorHandler, "GET", false);
        });
    }
    ...
    function handleScreenOptionsChange() {
        ...
        if ($("#adv-settings").length > 0) {
            ...
            activeOptions = [...(Array.isArray(meta_list_side) ? meta_list_side : []), ...(Array.isArray(meta_list_normal) ? meta_list_normal : [])];
    ...
    wps_js.handelMetaBoxFooter = function (key, response) {
        ...
        if (response.options && response.options.datepicker) {
            ...
            html +=
                `
                <button class="c-footer__filter__btn js-filters-toggle">` +
                dateFilterType +
                `</button>
                <div class="c-footer__filters">
                    <div class="c-footer__filters__current-filter">
                        <span class="c-footer__current-filter__title js-filter-title">` +
                dateFilterTitle +
                `</span>
                         <span class="c-footer__current-filter__date-range hs-filter-range">` +
                startDateResponse +
                " - " +
                endDateResponse +
                `</span>
                    ...
                    <div class="c-footer__filters__list">
                        <button data-metabox-key="${key}" data-filter="today" class="c-footer__filters__list-item">` +
                wps_js._("str_today") +
                `</button>
                        <button data-metabox-key="${key}" data-filter="yesterday" class="c-footer__filters__list-item">` +
                wps_js._("str_yesterday") +
                `</button>
                        <button data-metabox-key="${key}" data-filter="this_week" class="c-footer__filters__list-item">` +
                wps_js._("str_this_week") +
                `</button>
                        <button data-metabox-key="${key}" data-filter="last_week" class="c-footer__filters__list-item">` +
                wps_js._("str_last_week") +
                `</button>
                        <button data-metabox-key="${key}" data-filter="this_month" class="c-footer__filters__list-item">` +
                wps_js._("str_this_month") +
                `</button>
                        <button data-metabox-key="${key}" data-filter="last_month" class="c-footer__filters__list-item">` +
                wps_js._("str_last_month") +
                `</button>
                        <button class="c-footer__filters__list-item c-footer__filters__list-item--more js-show-more-filters">` +
                wps_js._("str_more") +
                `</button>
                        <div class="c-footer__filters__more-filters js-more-filters">
                            <button data-metabox-key="${key}" data-filter="7days" class="c-footer__filters__list-item">` +
                wps_js._("str_7days") +
                `</button>
                            <button data-metabox-key="${key}" data-filter="28days" class="c-footer__filters__list-item">` +
                wps_js._("str_28days") +
                `</button>
                            <button data-metabox-key="${key}" data-filter="30days" class="c-footer__filters__list-item">` +
                wps_js._("str_30days") +
                `</button>
                            <button data-metabox-key="${key}" data-filter="90days" class="c-footer__filters__list-item">` +
                wps_js._("str_90days") +
                `</button>
                            <button data-metabox-key="${key}" data-filter="6months" class="c-footer__filters__list-item">` +
                wps_js._("str_6months") +
                `</button>
                            <button data-metabox-key="${key}" data-filter="this_year" class="c-footer__filters__list-item">` +
                wps_js._("str_this_year") +
                `</button>
                            <button data-metabox-key="${key}" data-filter="last_year" class="c-footer__filters__list-item">` +
                wps_js._("str_last_year") +
                `</button>
                            <button class="c-footer__filters__close-more-filters js-close-more-filters">` +
                wps_js._("str_back") +
                `</button>
                        </div>
                        <input type="text" class="c-footer__filters__custom-date-input js-datepicker-input"/>
                        <button data-metabox-key="${key}" data-filter="custom" class="c-footer__filters__list-item c-footer__filters__list-item--custom js-custom-datepicker">` +
                wps_js._("str_custom") +
                `</button>
                    </div>
                </div> `;
