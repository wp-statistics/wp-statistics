// Move phpToMomentFormat to global scope
...
jQuery(document).ready(function () {
    ...
    if (datePickerBtn.length && datePickerElement.length && datePickerForm.length) {
        ...
        let ranges = {
            [wps_js._("str_today")]: [normalizeDate(localTime.clone(), validTimezone), normalizeDate(localTime.clone(), validTimezone)],
            [wps_js._("str_yesterday")]: [normalizeDate(localTime.clone().subtract(1, "days"), validTimezone), normalizeDate(localTime.clone().subtract(1, "days"), validTimezone)],
            [wps_js._("str_this_week")]: [normalizeDate(localTime.clone().startOf("week"), validTimezone), normalizeDate(localTime.clone().endOf("week"), validTimezone)],
            [wps_js._("str_last_week")]: [normalizeDate(localTime.clone().subtract(1, "week").startOf("week"), validTimezone), normalizeDate(localTime.clone().subtract(1, "week").endOf("week"), validTimezone)],
            [wps_js._("str_this_month")]: [normalizeDate(localTime.clone().startOf("month"), validTimezone), normalizeDate(localTime.clone().endOf("month"), validTimezone)],
            [wps_js._("str_last_month")]: [normalizeDate(localTime.clone().subtract(1, "month").startOf("month"), validTimezone), normalizeDate(localTime.clone().subtract(1, "month").endOf("month"), validTimezone)],
            [wps_js._("str_7days")]: [normalizeDate(localTime.clone().subtract(6, "days"), validTimezone), normalizeDate(localTime.clone(), validTimezone)],
            [wps_js._("str_28days")]: [normalizeDate(localTime.clone().subtract(27, "days"), validTimezone), normalizeDate(localTime.clone(), validTimezone)],
            [wps_js._("str_30days")]: [normalizeDate(localTime.clone().subtract(29, "days"), validTimezone), normalizeDate(localTime.clone(), validTimezone)],
            [wps_js._("str_90days")]: [normalizeDate(localTime.clone().subtract(89, "days"), validTimezone), normalizeDate(localTime.clone(), validTimezone)],
            [wps_js._("str_6months")]: [normalizeDate(localTime.clone().subtract(6, "months"), validTimezone), normalizeDate(localTime.clone(), validTimezone)],
            [wps_js._("str_year")]: [normalizeDate(localTime.clone().startOf("year"), validTimezone), normalizeDate(localTime.clone().endOf("year"), validTimezone)],
        };
        ...
        if (datePickerBtn.hasClass("js-date-range-picker-all-time")) {
            ...
            if (hasTypeParameter()) {
                post_date = wps_js.global.post_creation_date ? normalizeDate(moment(wps_js.global.post_creation_date), validTimezone) : normalizeDate(moment(0), validTimezone);
            } else {
                post_date = wps_js.global.initial_post_date ? normalizeDate(moment(wps_js.global.initial_post_date), validTimezone) : normalizeDate(moment(0), validTimezone);
            }
            ranges[wps_js._("all_time")] = [post_date, normalizeDate(moment(), validTimezone)];
        }
        ...
        if (wps_js.isset(wps_js.global, "request_params", "from") && wps_js.isset(wps_js.global, "request_params", "to")) {
            ...
            datePickerElement
                ...
                .container.find('.ranges li[data-range-key="' + defaultRange + '"]')
                .addClass("active");
