...
if ( $phrase ) {
	$q_string = $wpdb->prepare( "SELECT `words` , count(`words`) as `count` FROM `{$wpdb->prefix}statistics_search` WHERE `words` LIKE %s AND `words` <> '' AND `last_counter` BETWEEN %s AND %s GROUP BY `words` order by `count` DESC", '%' . $phrase . '%', $rangestartdate, $rangeenddate );
	$result = $wpdb->get_results( $q_string );
	$total = count( $result );
} else {
	$q_string = $wpdb->prepare( "SELECT `words` , count(`words`) as `count` FROM `{$wpdb->prefix}statistics_search` WHERE `words` <> '' AND `last_counter` BETWEEN %s AND %s GROUP BY `words` order by `count` DESC", $rangestartdate, $rangeenddate );
	$result = $wpdb->get_results( $q_string );
	$total = count( $result );
}
...
<div class="wrap">
    ...
    <div><?php wp_statistics_date_range_selector( WP_STATISTICS_SEARCHED_PHRASES_PAGE, $daysToDisplay, null, null, $phrase_field ); ?></div>
    ...
    <ul class="subsubsub">
		<?php if ( $phrase ) { ?>
            <li class="all"><a <?php if ( ! $phrase ) {
					echo 'class="current"';
				} ?>href="?page=<?php echo WP_STATISTICS_SEARCHED_PHRASES_PAGE . $date_args; ?>"><?php _e( 'All', 'wp-statistics' ); ?></a>
            </li>
            |
            <li>
                <a class="current" href="?page=<?php echo WP_STATISTICS_SEARCHED_PHRASES_PAGE; ?>&referr=<?php echo esc_html( $phrase ) . $date_args; ?>"> <?php echo htmlentities( $title, ENT_QUOTES ); ?>
                    <span class="count">(<?php echo $total; ?>)</span></a></li>
		<?php } else { ?>
            <li class="all"><a <?php if ( ! $phrase ) {
					echo 'class="current"';
				} ?>href="?page=<?php echo WP_STATISTICS_SEARCHED_PHRASES_PAGE . $date_args; ?>"><?php _e( 'All', 'wp-statistics' ); ?>
                    <span class="count">(<?php echo $total; ?>)</span></a></li>
		<?php } ?>
    </ul>
