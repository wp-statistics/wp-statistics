...
class Exclusion
{
    ...
    {
        // White list actions
        if (Helper::isBypassAdBlockersRequest() || Request::compare('action', 'wp_statistics_data_plus_event')) {
            return false;
        }
    ...
    public static function exclusion_admin_page($visitorProfile)
    {
        ...
        if (isset($_SERVER['SERVER_NAME']) and isset($requestUri)) {
            ...
            if (!Helper::isBypassAdBlockersRequest() && !Request::compare('action', 'wp_statistics_data_plus_event') && stripos($url, 'wp-admin') !== false) {
                return true;
            }
        }
