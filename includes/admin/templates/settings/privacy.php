    ...
    <table class="form-table">
        ...
        <tr valign="top">
            ...
            <td>
                ...
                <?php if ($isWpConsentApiActive) : ?>
                    <p class="description"><?php _e('<b>Note</b>: This integration requires a compatible consent management provider. If no compatible plugin is installed or active, all categories default to allowed (as though every visitor has opted in).', 'wp-statistics'); ?></p>

                    <?php
                        $compatiblePlugins = \WP_Statistics\Service\Integrations\WpConsentApi::getCompatiblePlugins();

                        if (empty($compatiblePlugins)) {
                            ?><p class="description"><?php esc_html_e('No Plugin Found: No recognized consent management plugin detected. Please install or activate a compatible plugin to ensure compliance with user consent.', 'wp-statistics'); ?></p><?php
                        } else {
                            ?><p class="description"><?php echo sprintf(esc_html__('Plugin Found: A recognized consent management plugin is active: %s. WP Statistics will comply with the user’s consent settings provided by this plugin.', 'wp-statistics'), implode(', ', $compatiblePlugins)); ?></p><?php
                        }
                    ?>
