    ...
    <div>
        ...
        <div class="wps-content-analytics-header__info">
            ...
            <span class="wps-content-analytics-header__author">
                <a href="<?php echo Menus::admin_url('author-analytics', ['type' => 'single-author', 'author_id' => $postAuthor, 'from' => Request::get('from', date('Y-m-d', strtotime('-30 days'))), 'to' => Request::get('to', date('Y-m-d'))]) ?>"><?php echo get_the_author_meta('display_name', $postAuthor) ?></a>
            </span>
        </div>
                ...
                foreach ($taxonomies as $taxonomy) {
                    ...
                    if ($terms) {
                        foreach ($terms as $term) {
                            echo '<a href="' . add_query_arg(['type' => 'single', 'term_id' => $term->term_id, 'from' => Request::get('from', date('Y-m-d', strtotime('-30 days'))), 'to' => Request::get('to', date('Y-m-d'))], Menus::admin_url('category-analytics')) . '">' . $term->name . '</a>';
                        }
                    }
                }
            ?>
