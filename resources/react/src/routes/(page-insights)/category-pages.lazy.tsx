...
import { getCategoryPagesQueryOptions, type CategoryPageRecord } from '@/services/page-insight/get-category-pages'
...
function RouteComponent() {
  ...
  const columns = useMemo<ColumnDef<CategoryPageRecord>[]>(
    () => [
      {
        accessorKey: 'page_title',
        header: __('Category', 'wp-statistics'),
        enableSorting: false,
        cell: ({ row }) => {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-right">
            <span className="font-medium">{formatCompactNumber(Number(row.original.visitors))}</span>
          </div>
        ),
      },
    ],
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-input">
        <h1 className="text-xl font-semibold text-neutral-800">{__('Category Pages', 'wp-statistics')}</h1>
