  ...
  const columns = useMemo<ColumnDef<EntryPageRecord>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-right">
            <span className="font-medium">{formatDecimal(Number(row.original.bounce_rate) || 0)}%</span>
          </div>
        ),
      },
    ],
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-input">
        <h1 className="text-xl font-semibold text-neutral-800">{__('Entry Pages', 'wp-statistics')}</h1>
