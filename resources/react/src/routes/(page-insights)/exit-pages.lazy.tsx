...
import { getExitPagesQueryOptions, type ExitPageRecord } from '@/services/page-insight/get-exit-pages'
...
function RouteComponent() {
  ...
  const columns = useMemo<ColumnDef<ExitPageRecord>[]>(
    () => [
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-right">
            <span className="font-medium">{formatCompactNumber(Number(row.original.sessions))}</span>
          </div>
        ),
      },
    ],
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-input">
        <h1 className="text-xl font-semibold text-neutral-800">{__('Exit Pages', 'wp-statistics')}</h1>
      ...
      <div className="p-2">
        ...
        {isError ? (
          <div className="p-2 text-center">
            <ErrorMessage message={__('Failed to load exit pages', 'wp-statistics')} />
          ...
          <DataTable
            ...
            emptyStateMessage={__('No exit pages found for the selected period', 'wp-statistics')}
            stickyHeader={true}
          />
        )}
      </div>
