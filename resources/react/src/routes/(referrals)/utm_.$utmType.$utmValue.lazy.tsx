    ...
    <Panel className="p-8 text-center">
      <div className="max-w-md mx-auto space-y-4">
        ...
        <h2 className="text-lg font-semibold text-neutral-800">
          {__('UTM Detail Report', 'wp-statistics')}
        </h2>
        <p className="text-sm text-muted-foreground">
          {__(
            'Get detailed analytics for individual UTM parameters including traffic trends, referrers, sources, mediums, and entry pages.',
            'wp-statistics'
          )}
        </p>
        <p className="text-sm text-muted-foreground">
        ...
        <a
          href="https://wp-statistics.com/pricing/?utm_source=plugin&utm_medium=link&utm_campaign=utm-detail"
          target="_blank"
...
function RouteComponent() {
  ...
  useEffect(() => {
    const handleContentRegistered = (event: CustomEvent) => {
      if (event.detail?.pageId === 'single-utm') {
        forceUpdate((n) => n + 1)
      }
    }
  ...
  const pageContent = getPageContent('single-utm')
  if (pageContent?.render) {
    const premiumContent = pageContent.render({ utmType, utmValue })
    if (premiumContent) {
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="p-3">
        <NoticeContainer className="mb-2" currentRoute="single-utm" />
        <LockedState />
      </div>
    </div>
  )
