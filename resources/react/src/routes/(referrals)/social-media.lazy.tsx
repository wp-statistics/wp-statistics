...
function RouteComponent() {
  ...
  const handleSocialTypeChange = useCallback(
    (value: string) => {
      setSocialType(value as SocialType)
      setPage(1)
    },
    [setPage]
  )
    ...
    () => [
      {
        ...
        value: socialType,
        options: [
          { value: 'all', label: __('All Social', 'wp-statistics') },
          { value: 'organic', label: __('Organic Social', 'wp-statistics') },
          { value: 'paid', label: __('Paid Social', 'wp-statistics') },
        ],
        onChange: handleSocialTypeChange,
      },
    ],
    [socialType, handleSocialTypeChange]
  )
  ...
  // Define columns
  const columns = useMemo<ColumnDef<SocialMediaRow>[]>(
    () => [
      {
        accessorKey: 'referrer_domain',
        header: __('Platform', 'wp-statistics'),
        enableSorting: true,
        cell: ({ row }) => {
    ...
    <div className="min-w-0">
      ...
      <div className="flex items-center justify-between px-4 py-3 ">
        ...
        <div className="flex items-center gap-2">
          <Select value={socialType} onValueChange={handleSocialTypeChange}>
            <SelectTrigger className="h-8 w-auto text-xs gap-1 px-3 border-neutral-200 hover:bg-neutral-50">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{__('All Social', 'wp-statistics')}</SelectItem>
              <SelectItem value="organic">{__('Organic Social', 'wp-statistics')}</SelectItem>
        ...
        {isError ? (
          ...
          <>
            ...
            <DataTable
              columns={columns}
              data={tableData}
              title={__('Social Media Platforms', 'wp-statistics')}
              sorting={sorting}
              ...
              emptyStateMessage={__('No social media platforms found for the selected period', 'wp-statistics')}
              stickyHeader={true}
            />
          </>
        )}
