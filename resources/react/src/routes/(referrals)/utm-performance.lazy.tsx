    ...
    <Panel className="p-8 text-center">
      <div className="max-w-md mx-auto space-y-4">
        ...
        <h2 className="text-lg font-semibold text-neutral-800">
          {__('UTM Performance Report', 'wp-statistics')}
        </h2>
        <p className="text-sm text-muted-foreground">
          {__(
            'Track your UTM campaigns, sources, and mediums with detailed performance reports. Monitor visitor engagement across all your marketing channels.',
            'wp-statistics'
          )}
        </p>
        <p className="text-sm text-muted-foreground">
        ...
        <a
          href="https://wp-statistics.com/pricing/?utm_source=plugin&utm_medium=link&utm_campaign=utm-performance"
          target="_blank"
...
function RouteComponent() {
  ...
  useEffect(() => {
    const handleContentRegistered = (event: CustomEvent) => {
      if (event.detail?.pageId === 'utm-performance') {
        forceUpdate((n) => n + 1)
      }
    }
  ...
  const pageContent = getPageContent('utm-performance')
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-center justify-between px-4 py-3">
        <h1 className="text-2xl font-semibold text-neutral-800">{__('UTM Performance', 'wp-statistics')}</h1>
      </div>
      <div className="p-3">
        <NoticeContainer className="mb-2" currentRoute="utm-performance" />
        <LockedState />
      </div>
    </div>
  )
