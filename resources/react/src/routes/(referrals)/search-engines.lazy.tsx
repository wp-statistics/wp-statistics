...
function RouteComponent() {
  ...
  // Search type filter (All/Organic/Paid) - shown in header AND Options drawer
  const {
    value: searchType,
    onChange: onSearchTypeChange,
    options: searchTypeOptions,
    getApiFilter,
    pageFilterConfig: searchTypeFilterConfig,
  } = useSearchTypeFilter()
  ...
  // Get the search type API filter
  const searchTypeApiFilter = useMemo(() => getApiFilter(), [getApiFilter])
  ...
  const {
    ...
    ...getSearchEnginesOverviewQueryOptions({
      ...
      filters: appliedFilters,
      apiFilters: searchTypeApiFilter,
      page,
      perPage: PER_PAGE,
      orderBy,
      order: order as 'asc' | 'desc',
      context: SEARCH_ENGINES_CONTEXT,
    }),
  ...
  const chartResponse = response?.data?.items?.chart as SearchEngineChartResponse | undefined
  const tableResponse = response?.data?.items?.table as SearchEngineTableResponse | undefined

  // Build dynamic chart metrics from datasets (Google, Bing, Yahoo, Total)
  ...
  const { chartData, chartMetrics } = useMemo(() => {
    ...
    // Define chart colors for dynamic search engines + total
    const chartColors = [
      'var(--chart-1)',
      'var(--chart-2)',
      'var(--chart-3)',
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="p-3">
        <NoticeContainer className="mb-2" currentRoute="search-engines" />

        {isError ? (
          <div className="p-2 text-center">
            <ErrorMessage message={__('Failed to load search engines data', 'wp-statistics')} />
          ...
          <div className="space-y-3">
            {/* Search Engines Chart - shows top 3 engines + total */}
            <LineChart
