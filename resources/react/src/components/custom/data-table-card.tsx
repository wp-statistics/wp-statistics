  ...
  return (
    <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        >
          {displayBodyColumns.map((col) => (
            <div key={col.id || (col as { accessorKey?: string }).accessorKey} className="text-center">
              <div className="text-xs text-muted-foreground mb-0.5">{getColumnHeader(col)}</div>
      ...
      {isExpanded && displaySecondaryColumns.length > 0 && (
        <div className="border-t border-border/50 px-3 py-2 space-y-2 bg-muted/50">
          {displaySecondaryColumns.map((col) => (
            <div
              key={col.id || (col as { accessorKey?: string }).accessorKey}
              className="flex justify-between items-center gap-2"
            >
              <span className="text-xs text-muted-foreground shrink-0">{getColumnHeader(col)}</span>
