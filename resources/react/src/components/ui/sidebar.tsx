...
function SidebarProvider({
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    () => ({
      ...
      open,
      setOpen,
      isMobile: isMobileOrTablet, // Use isMobileOrTablet for sidebar Sheet behavior
      openMobile,
      setOpenMobile,
      toggleSidebar,
    }),
...
function Sidebar({
  ...
  return (
    <div
      className="group peer text-sidebar-foreground hidden h-full lg:block"
      data-state={state}
    ...
    >
      ...
      <div
        ...
        className={cn(
          'absolute inset-y-0 z-10 hidden w-(--sidebar-width) bg-sidebar transition-[left,right,width] duration-200 ease-linear lg:flex',
          side === 'left'
      ...
      >
        <div
          ...
          className="group-data-[variant=floating]:border-sidebar-border sticky top-0 flex w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm h-[calc(100vh-var(--wp-admin-bar-height)-var(--header-height))]"
        >
          {children}
        </div>
      </div>
...
function SidebarInset({ className, ...props }: React.ComponentProps<'main'>) {
  return (
    <main
      ...
      className={cn(
        'bg-neutral-100 relative flex min-w-0 flex-1 flex-col overflow-hidden',
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        // Increases the hit area of the button on mobile/tablet.
