/**
 * Column definitions for region-based data tables.
 * Geographic analytics grouped by region (US states, provinces, etc.).
 *
 * This module is designed for reuse across multiple region-based reports
 * (e.g., US States, Regions by Country) with configurable column titles.
 */
...
import type { USStateRecord } from '@/services/geographic/get-us-states'

/**
 * Context identifiers for user preferences
 */
export const REGION_CONTEXT_US_STATES = 'us-states'

/**
 * Columns hidden by default (can be shown via column management)
 */
export const REGION_DEFAULT_HIDDEN_COLUMNS: string[] = ['bounceRate', 'sessionDuration', 'percentOfTotal']
 ...
 */
export const REGION_DEFAULT_COMPARISON_COLUMNS: string[] = ['visitors']

/**
 * Columns that support PP comparison display
 */
export const REGION_COMPARABLE_COLUMNS: string[] = [
  'visitors',
  'views',
  'viewsPerVisitor',
  'bounceRate',
 ...
 */
export const REGION_COLUMN_CONFIG: ColumnConfig = {
 ...
 */
export const REGION_DEFAULT_API_COLUMNS = getDefaultApiColumns(REGION_COLUMN_CONFIG)

/**
 * Region data interface for the table
 * Uses generic region terminology for reusability across different region types
 */
...
/**
 * Transform API response to RegionData interface
 */
export function transformRegionData(record: USStateRecord, totalVisitors: number): RegionData {
  const previous = record.previous
  const visitors = Number(record.visitors) || 0
  const regionCode = record.region_code || '000'

  return {
    id: `region-${regionCode}`,
    regionCode,
    regionName: record.region_name || 'Unknown',
    visitors,
...
/**
 * Options for creating region columns
 */
export interface RegionColumnsOptions {
  /** Column header title - "State", "Region", "Province" (default: "Region") */
  regionTitle?: string
  /** Date range comparison label for tooltip (from useComparisonDateLabel) */
  comparisonLabel?: string
  /** Columns that should display PP comparison (defaults to REGION_DEFAULT_COMPARISON_COLUMNS) */
  comparisonColumns?: string[]
}

/**
 * Create column definitions for region-based tables
 */
export function createRegionColumns(options: RegionColumnsOptions): ColumnDef<RegionData>[] {
  const {
    regionTitle = __('Region', 'wp-statistics'),
    comparisonLabel,
    comparisonColumns = REGION_DEFAULT_COMPARISON_COLUMNS,
  } = options
  ...
  return [
    {
      ...
      cell: ({ row }) => (
        <span className="text-xs font-medium text-neutral-700 truncate">{row.original.regionName}</span>
      ),
      enableSorting: false,
      meta: {
        title: regionTitle,
