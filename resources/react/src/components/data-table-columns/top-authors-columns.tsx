 ...
 * Content-based analytics grouped by author.
 */
...
import { AuthorCell, DurationCell, NumericCell } from '@/components/data-table-columns'
 ...
 * Shows 5 columns by default: author, visitors, views, published, sessionDuration
 */
export const TOP_AUTHORS_DEFAULT_HIDDEN_COLUMNS: string[] = ['viewsPerContent', 'bounceRate']
 ...
 */
export const TOP_AUTHORS_DEFAULT_COMPARISON_COLUMNS: string[] = ['visitors']

/**
 * Columns that support PP comparison display
 */
export const TOP_AUTHORS_COMPARABLE_COLUMNS: string[] = [
  'visitors',
  'views',
  'published',
  'viewsPerContent',
...
export function transformTopAuthorData(record: TopAuthorRecord): TopAuthor {
  ...
  return {
    ...
    authorName: record.author_name || __('Unknown Author', 'wp-statistics'),
...
/**
 * Options for creating Top Authors columns
 */
export interface TopAuthorsColumnsOptions {
  /** Date range comparison label for tooltip (from useComparisonDateLabel) */
  comparisonLabel?: string
  /** Columns that should display PP comparison (defaults to TOP_AUTHORS_DEFAULT_COMPARISON_COLUMNS) */
 ...
 */
export function createTopAuthorsColumns(options: TopAuthorsColumnsOptions = {}): ColumnDef<TopAuthor>[] {
  const { comparisonLabel, comparisonColumns = TOP_AUTHORS_DEFAULT_COMPARISON_COLUMNS } = options
