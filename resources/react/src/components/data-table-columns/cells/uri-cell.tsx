...
export const UriCell = memo(function UriCell({ uri, maxLength = 50 }: UriCellProps) {
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <span className="text-xs font-mono text-foreground cursor-default">{truncatedUri}</span>
        </TooltipTrigger>
  ...
  return <span className="text-xs font-mono text-foreground">{displayUri}</span>
})
