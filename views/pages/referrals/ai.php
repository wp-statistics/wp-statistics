<?php
use WP_Statistics\Components\View;

$isLocked = apply_filters('wp_statistics_referrals_ai_locked', true);

if ($isLocked) {
    $args = [
        'page_title'        => esc_html__('AI Insights Add-on', 'wp-statistics'),
        'page_second_title' => esc_html__('WP Statistics Premium: Unlock More Than Just AI Insights', 'wp-statistics'),
        'addon_name'        => esc_html__('AI Insights', 'wp-statistics'),
        'addon_slug'        => 'wp-statistics-ai-insights',
        'campaign'          => 'ai-insights',
        'more_title'        => esc_html__('Learn More About the AI Insights Add-on', 'wp-statistics'),
        'premium_btn_title' => esc_html__('Upgrade Now to Unlock All Premium Features!', 'wp-statistics'),
        'images'            => ['marketing-lock.png','campaign-lock.png','goals-lock.png'],
        'description'       => esc_html__('The AI Insights add-on is a premium feature for WP Statistics that brings powerful marketing insights directly to your dashboard. ', 'wp-statistics'),
        'second_description'=> esc_html__('When you upgrade to WP Statistics Premium, you don’t just get access to the AI insights add-on—you unlock all premium add-ons, delivering advanced analytics tools and detailed reports for every aspect of your site.', 'wp-statistics')
    ];
    View::load("pages/lock-page", $args);
}