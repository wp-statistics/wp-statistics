...
class ExportSettingsRestController extends BaseRestAPI
{
    /**
     * ExportSettingsRestController constructor.
     */
    public function __construct()
    {
        parent::__construct();

        $this->endpoint = 'export-settings';
    }


    /**
     * Checks whether the current user has permission to access export settings.
     *
     * This method is used as the permission_callback for export-settings routes.
     *
    ...
    public function permissionCallback(WP_REST_Request $request)
    {
        if (!User::Access('manage')) {
            return new WP_Error(
                'rest_cannot_access',
                __('Sorry, you are not authorized to Export the settings.', 'wp-statistics'),
    ...
    /**
     * Handle the export-settings REST API request.
     *
     * This method will process the actual export of settings.
