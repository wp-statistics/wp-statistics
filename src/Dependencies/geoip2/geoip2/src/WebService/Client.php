...
namespace WP_Statistics\Dependencies\GeoIp2\WebService;

use WP_Statistics\Dependencies\GeoIp2\Exception\AddressNotFoundException;
use WP_Statistics\Dependencies\GeoIp2\Exception\AuthenticationException;
use WP_Statistics\Dependencies\GeoIp2\Exception\GeoIp2Exception;
use WP_Statistics\Dependencies\GeoIp2\Exception\HttpException;
use WP_Statistics\Dependencies\GeoIp2\Exception\InvalidRequestException;
use WP_Statistics\Dependencies\GeoIp2\Exception\OutOfQueriesException;
use WP_Statistics\Dependencies\GeoIp2\Model\City;
use WP_Statistics\Dependencies\GeoIp2\Model\Country;
use WP_Statistics\Dependencies\GeoIp2\Model\Insights;
use WP_Statistics\Dependencies\GeoIp2\ProviderInterface;
use WP_Statistics\Dependencies\MaxMind\WebService\Client as WsClient;
...
class Client implements ProviderInterface
{
     ...
     *                          from will be used.
     *
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\AddressNotFoundException if the address you
     *                                                    provided is not in our database (e.g., a private address).
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\AuthenticationException  if there is a problem
     *                                                    with the account ID or license key that you provided
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\OutOfQueriesException    if your account is out
     *                                                    of queries
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\InvalidRequestException} if your request was received by the web service but is
     *                                                    invalid for some other reason.  This may indicate an issue
     *                                                    with this API. Please report the error to MaxMind.
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\HttpException   if an unexpected HTTP error code or message was returned.
     ...
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\GeoIp2Exception This serves as the parent
     ...
     *                          from will be used.
     *
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\AddressNotFoundException if the address you provided is not in our database (e.g.,
     *                                                    a private address).
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\AuthenticationException  if there is a problem
     *                                                    with the account ID or license key that you provided
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\OutOfQueriesException    if your account is out of queries
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\InvalidRequestException} if your request was received by the web service but is
     *                                                    invalid for some other reason.  This may indicate an
     *                                                    issue with this API. Please report the error to MaxMind.
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\HttpException   if an unexpected HTTP error
     ...
     *                                           error code.
     * @throws \WP_Statistics\Dependencies\GeoIp2\Exception\GeoIp2Exception This serves as the parent class to the above exceptions. It
    ...
    private function responseFor(string $endpoint, string $class, string $ipAddress): Country
    {
        ...
        } catch (\WP_Statistics\Dependencies\MaxMind\Exception\IpAddressNotFoundException $ex) {
            throw new AddressNotFoundException(
                $ex->getMessage(),
                $ex->getStatusCode(),
                $ex
            );
        } catch (\WP_Statistics\Dependencies\MaxMind\Exception\AuthenticationException $ex) {
            throw new AuthenticationException(
                $ex->getMessage(),
                $ex->getStatusCode(),
                $ex
            );
        } catch (\WP_Statistics\Dependencies\MaxMind\Exception\InsufficientFundsException $ex) {
            throw new OutOfQueriesException(
                $ex->getMessage(),
                $ex->getStatusCode(),
                $ex
            );
        } catch (\WP_Statistics\Dependencies\MaxMind\Exception\InvalidRequestException $ex) {
        ...
        } catch (\WP_Statistics\Dependencies\MaxMind\Exception\HttpException $ex) {
        ...
        } catch (\WP_Statistics\Dependencies\MaxMind\Exception\WebServiceException $ex) {
