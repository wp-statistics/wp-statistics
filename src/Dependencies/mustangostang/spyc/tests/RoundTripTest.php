...
function roundTrip($a) { return \WP_Statistics_Spyc::YAMLLoad(\WP_Statistics_Spyc::YAMLDump(array('x' => $a))); }


class WP_Statistics_RoundTripTest extends PHPUnit_Framework_TestCase {
    ...
    public function testABCD() {
      $this->assertEquals (array ('a', 'b', 'c', 'd'), \WP_Statistics_Spyc::YAMLLoad(\WP_Statistics_Spyc::YAMLDump(array('a', 'b', 'c', 'd'))));
    }
    
    public function testABCD2() {
        $a = array('a', 'b', 'c', 'd'); // Create a simple list
        $b = \WP_Statistics_Spyc::YAMLDump($a);        // Dump the list as YAML
        $c = \WP_Statistics_Spyc::YAMLLoad($b);        // Load the dumped YAML
        $d = \WP_Statistics_Spyc::YAMLDump($c);        // Re-dump the data
        $this->assertSame($b, $d);
    }
