...
class WP_Statistics_DumpTest extends PHPUnit_Framework_TestCase {
    ...
    public function testDump() {
      foreach ($this->files_to_test as $file) {
        ...
        $dump = \WP_Statistics_Spyc::YAMLDump ($yaml);
        $yaml_after_dump = \WP_Statistics_Spyc::YAMLLoad ($dump);
        $this->assertEquals ($yaml, $yaml_after_dump);
      }
    }

    public function testDumpWithQuotes() {
      $Spyc = new \WP_Statistics_Spyc();
    ...
    public function testDumpArrays() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('item1', 'item2', 'item3'));
      $awaiting = "---\n- item1\n- item2\n- item3\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testNull() {
        $dump = \WP_Statistics_Spyc::YAMLDump(array('a' => 1, 'b' => null, 'c' => 3));
        $awaiting = "---\na: 1\nb: null\nc: 3\n";
        $this->assertEquals ($awaiting, $dump);
    }
    ...
    public function testNext() {
        ...
        next($array);
        $dump = \WP_Statistics_Spyc::YAMLDump($array);
        $awaiting = "---\n- aaa\n- bbb\n- ccc\n";
        $this->assertEquals ($awaiting, $dump);
    }
    ...
    public function testDumpingMixedArrays() {
        ...
        $yaml = \WP_Statistics_Spyc::YAMLDump($array,4,60);
    }

    public function testMixed() {
        $dump = \WP_Statistics_Spyc::YAMLDump(array(0 => 1, 'b' => 2, 1 => 3));
        $awaiting = "---\n0: 1\nb: 2\n1: 3\n";
        $this->assertEquals ($awaiting, $dump);
    }

    public function testDumpNumerics() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('404', '405', '500'));
      $awaiting = "---\n- \"404\"\n- \"405\"\n- \"500\"\n";
      $this->assertEquals ($awaiting, $dump);
    }
    ...
    public function testDumpAmpersands() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('some' => '&foo'));
      $awaiting = "---\nsome: '&foo'\n";
      $this->assertEquals ($awaiting, $dump);
    }
    ...
    public function testDumpApostrophes() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('some' => "'Biz' pimpt bedrijventerreinen"));
      $awaiting = "---\nsome: \"'Biz' pimpt bedrijventerreinen\"\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testDumpNumericHashes() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ("titel"=> array("0" => "", 1 => "Dr.", 5 => "Prof.", 6 => "Prof. Dr.")));
      $awaiting = "---\ntitel:\n  0: \"\"\n  1: Dr.\n  5: Prof.\n  6: Prof. Dr.\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testEmpty() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array("foo" => array()));
      $awaiting = "---\nfoo: [ ]\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testHashesInKeys() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('#color' => '#ffffff'));
      $awaiting = "---\n\"#color\": '#ffffff'\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testParagraph() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('key' => "|\n  value"));
      $awaiting = "---\nkey: |\n  value\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testParagraphTwo() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('key' => 'Congrats, pimpt bedrijventerreinen pimpt bedrijventerreinen pimpt bedrijventerreinen!'));
      $awaiting = "---\nkey: >\n  Congrats, pimpt bedrijventerreinen pimpt\n  bedrijventerreinen pimpt\n  bedrijventerreinen!\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testString() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('key' => array('key_one' => 'Congrats, pimpt bedrijventerreinen!')));
      $awaiting = "---\nkey:\n  key_one: Congrats, pimpt bedrijventerreinen!\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testStringLong() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('key' => array('key_one' => 'Congrats, pimpt bedrijventerreinen pimpt bedrijventerreinen pimpt bedrijventerreinen!')));
      $awaiting = "---\nkey:\n  key_one: >\n    Congrats, pimpt bedrijventerreinen pimpt\n    bedrijventerreinen pimpt\n    bedrijventerreinen!\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testStringDoubleQuote() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('key' => array('key_one' =>  array('key_two' => '"Système d\'e-réservation"'))));
      $awaiting = "---\nkey:\n  key_one:\n    key_two: |\n      Système d'e-réservation\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testLongStringDoubleQuote() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('key' => array('key_one' =>  array('key_two' => '"Système d\'e-réservation bedrijventerreinen pimpt" bedrijventerreinen!'))));
    ...
    public function testPerCentOne() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('key' => "%name%, pimpts bedrijventerreinen!"));
      $awaiting = "---\nkey: '%name%, pimpts bedrijventerreinen!'\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testPerCentAndSimpleQuote() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('key' => "%name%, pimpt's bedrijventerreinen!"));
      $awaiting = "---\nkey: \"%name%, pimpt's bedrijventerreinen!\"\n";
      $this->assertEquals ($awaiting, $dump);
    }

    public function testPerCentAndDoubleQuote() {
      $dump = \WP_Statistics_Spyc::YAMLDump(array ('key' => '%name%, pimpt\'s "bed"rijventerreinen!'));
