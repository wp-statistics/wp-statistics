 ...
 */
class WP_Statistics_Test_WP_Background_Process extends WP_UnitTestCase {
	/**
	...
	public function setUp(): void {
		...
		$this->wpbp = $this->getMockForAbstractClass( \WP_Statistics_WP_Background_Process::class );
	...
	private function getWPBPProperty( string $name ) {
		try {
			$property = new ReflectionProperty( 'WP_Statistics_WP_Background_Process', $name );
		} catch ( Exception $e ) {
	...
	private function executeWPBPMethod( string $name, ...$args ) {
		try {
			$method = new ReflectionMethod( 'WP_Statistics_WP_Background_Process', $name );
	...
	public function test_push_to_queue() {
		$this->assertClassHasAttribute( 'data', 'WP_Statistics_WP_Background_Process', 'class has data property' );
	...
	public function test_get_batch() {
		...
		$batch_data_object = new \WP_Statistics_Test_Batch_Data();
		...
		$this->assertInstanceOf( \WP_Statistics_Test_Batch_Data::class, $third_batch->data[0] );

		// Explicitly set allowed classes to Test_Batch_Data.
		$this->setWPBPProperty( 'allowed_batch_data_classes', array( \WP_Statistics_Test_Batch_Data::class ) );
		$third_batch = $this->executeWPBPMethod( 'get_batch' );
		$this->assertCount( 1, $third_batch->data );
		$this->assertInstanceOf( \WP_Statistics_Test_Batch_Data::class, $third_batch->data[0] );
	...
	public function test_maybe_handle_cancelled() {
		...
		$this->wpbp->save();
		$this->assertCount( 2, $this->wpbp->get_batches() );
		update_site_option( $this->executeWPBPMethod( 'get_status_key' ), \WP_Statistics_WP_Background_Process::STATUS_CANCELLED );
